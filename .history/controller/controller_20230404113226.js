...
addNhanVien = () => {
  ...
  //   Check bỏ trống;
  valid =
    required(addNV.taiKhoan, "tknv", "error-taikhoan") &
    required(addNV.hoTen, "name", "error-name") &
    required(addNV.email, "email", "error-email") & 
    required(addNV.ngayLam, "email", "error-ngaylam") & 
    required(addNV.password, "password", "error-matkhau") &
    required(addNV.luong, "password", "error-luong") &
    required(addNV.gioLam, "password", "error-giolam");
  // Check độ dài
  valid = valid & lengthCheck(addNV.taiKhoan, "tknv", "error-taikhoan", 4, 6);
  // Check email
  valid = valid & checkEmail(addNV.email, "email", "error-email");

  // Check lương
  valid = valid & checkLuong(addNV.luong, "luongCB", "error-luong");

  // Check lương vào giờ làm
  valid = valid & checkGioLam(addNV.gioLam, "gioLam", "error-giolam");
